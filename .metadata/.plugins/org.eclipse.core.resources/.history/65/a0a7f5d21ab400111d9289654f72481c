...
/* Apogee detection                                                    */
/*                                                                     */
/* Method: watch altitude from the barometric pressure sensor, backed  */
/* up by vertical velocity from dead reckoning. Baro altitude is used  */
/* as the primary signal because double-integrated accelerometer data */
/* drifts too much over a multi-second flight to trust alone — this is */
/* the same reason real hobby/HPR altimeters use a barometer for       */
/* apogee detection rather than pure dead reckoning.                   */
/*                                                                     */
/* A single falling sample is not trusted on its own — pressure sensor */
/* noise near apogee (where vertical speed, and thus the pressure      */
/* rate of change, is near zero) can cause a false trigger. Instead we */
